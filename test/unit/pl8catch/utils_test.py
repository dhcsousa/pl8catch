...
def test_detect_plate(config, models, test_image):
    ...
    assert isinstance(detected_car.object_biding_box, tuple)
    assert len(detected_car.object_biding_box) == 4
    assert isinstance(detected_car.plate_biding_box, tuple)
    assert len(detected_car.plate_biding_box) == 4
...
def sample_detected_objects():
    ...
    detected_objects = [
        DetectedObject(
            ...
            object_biding_box=(50, 50, 150, 150),
            plate_biding_box=(70, 70, 120, 90),
        ),
        DetectedObject(
            object_id=2,
            ...
            object_biding_box=(200, 200, 300, 300),
            plate_biding_box=(220, 220, 280, 240),
        ),
    ]
